...
- [x] Live auf Vercel: **https://frostbreeze-shop.vercel.app**. **Abweichung:** Account ist `zapkothimofej-2616` (Sid-Vorgabe; Thimorrow-Regel gilt nur fuer die Git-Identitaet). Das erste Projekt `frostbreeze` war vom Abuse-Scanner geflaggt (451 DEPLOYMENT_DISABLED auf jeder Version) -> frisches Projekt `frostbreeze-shop` + public Repo (github.com/Thimorrow/frostbreeze) + Git-Integration. **Beweis:** Deploy "Ready"; curl Home/PDP/Suche/Favicon -> 200; Playwright-Klick-Flow LIVE gruen: ATC disabled->enabled, Drawer + Gesamt, CHECKOUT-URL `https://frostbreeze-store.myshopify.com/checkouts/cn/...`, Konto-Login mit E-Mail-Feld, mobile Sticky-Buy-Bar oeffnet Drawer, 404 echt (Status 404).
- [x] `live-verifier`-Agent liefert PASS auf der Live-URL. **Beweis:** Erster Lauf 7/8 — fand echten Bug (Soft-404: globales PPR sendete Shell immer mit Status 200, notFound() konnte nichts mehr aendern; Fix `ppr: "incremental"` + Opt-in nur Home/Suche, Commit 45e3fbc). Re-Verifikation danach: **PASS** — `/gibtesnicht` 404, `/product/gibtsnicht` 404, Home/Suche/PDP 200 ohne Regression, Antwortzeiten < 1s.
